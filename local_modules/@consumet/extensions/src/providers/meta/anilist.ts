...
import { AnimeParser, MediaStatus, SubOrSub, Genres, MediaFormat } from '../../models';
import {
  anilistSearchQuery,
...
class Anilist extends AnimeParser {
    ...
    if (malId) {
      ...
      if (malAsyncReq.status === 200) {
        ...
        const possibleSource = sites.find(
          s => s.page.toLowerCase() === provider.name.toLowerCase(),
        );
  ...
  private findMangaRaw = async (provider: MangaParser, slug: string, title: string) => {
    ...
    if (!possibleManga)
      return (await provider.fetchMangaInfo(findAnime.results[0].id)) as IMangaInfo;
