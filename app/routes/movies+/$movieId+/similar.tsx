...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `AnimeNexus - ${movieTitle} - Similar` },
      {
        property: 'og:url',
        content: `https://anime-nexus-six.vercel.app/movies/${params.movieId}/similar`,
      },
      { property: 'og:title', content: `AnimeNexus - ${movieTitle} - Similar` },
      { name: 'twitter:title', content: `AnimeNexus - ${movieTitle} - Similar` },
    ];
  },
);
