...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `AnimeNexus - ${name} ${seasonDetail?.name || ''}` },
    {
      property: 'og:url',
      content: `https://anime-nexus-six.vercel.app/tv-shows/${params.tvId}/season/${params.seasonId}/`,
    },
    { property: 'og:title', content: `AnimeNexus - ${name} ${seasonDetail?.name || ''}` },
    { name: 'twitter:title', content: `AnimeNexus - ${name} ${seasonDetail?.name || ''}` },
  ];
});
